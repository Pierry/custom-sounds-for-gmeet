...
(function () {
  ...
  function openDialog() {
    ...
    var foot = mk('div', 'display:flex;justify-content:flex-end;gap:8px');
  ...
  function panel() {
    ...
    el.id = 'mos-panel';
    el.appendChild(mk('div', 'font-weight:600;margin-bottom:8px', 'Custom Sounds for GMeet'));
    ...
    var r1 = mk('div', 'display:flex;gap:6px;margin-bottom:8px');
    ...
    var r2 = mk('div', 'display:flex;gap:6px');
    var tg = mk('button', B, ''), cl = mk('button', 'width:34px;background:#33333f;border:0;color:#e6e6ee;border-radius:8px;cursor:pointer', 'x');
    function refresh() { tg.textContent = localStorage.getItem('mos_enabled') === '0' ? 'Off' : 'On'; }
    tg.addEventListener('click', function () { ls('mos_enabled', localStorage.getItem('mos_enabled') === '0' ? '' : '0'); refresh(); });
    cl.addEventListener('click', function () { el.style.display = 'none'; });
    r2.appendChild(tg); r2.appendChild(cl); el.appendChild(r2);
    document.body.appendChild(el); refresh();
  }
